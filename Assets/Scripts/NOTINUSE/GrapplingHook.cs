...
public class GrapplingHook : MonoBehaviour
{
    ...
    void Start()
    {
        this.CharContrl = gameObject.GetComponent<CharacterController>();
    ...
    void FixedUpdate()
    {
        ...
        if (hooked_node != null && Input.GetMouseButton(0))
        {
            ...
            if (disconent_on_los_break && Physics.Raycast(ray, out hit) && hit.collider.tag != "grappling_node") //"GRAPPLING_NODE_TAG")
            {
                Debug.Log("LOS Break");
                BreakTether();
                return;
            ...
            if (CharContrl.velocity.magnitude > break_tether_velo)
            {
                BreakTether();
                return;
            }
            ...
            Vector3 curr_velo_upf = CharContrl.velocity * Time.fixedDeltaTime;
    ...
    private void ApplyTensionForce(Vector3 curr_velo_upf, Vector3 test_pos)
    {
      ...
      //Vector3 tension_force = CharContrl.mass * (delta_velocity / Time.fixedDeltaTime);

        //CharContrl.AddForce(tension_force, ForceMode.Impulse);
    }
